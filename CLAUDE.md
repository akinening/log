...
- **Content collection `works`** (`src/content/works/*.md`, schema in `src/content/config.ts`): each file is one case study with required frontmatter (`title`, `summary`, `company`, `period`, `team`, `role`, `year`, `thumbnail`, `tags`, 1–3 `metrics` entries, `order`) plus an optional `tldr` (`problem`/`approach`/`outcome` — rendered as a 30-second summary block on the case page). Metrics are for outcomes only; don't pad with scope-of-work numbers to reach 3. Markdown body is rendered as the case study's prose. `order` controls display order everywhere (not filename or date).
- **Routing**: `src/pages/index.astro` (top page, lists works + speaking/writing), `src/pages/experience.astro` (career history), `src/pages/works/[slug].astro` (dynamic case-study page generated per collection entry via `getStaticPaths`, with a "next case" link that wraps around the sorted work list), `src/pages/resume.astro` (print/PDF-oriented 職務経歴書 — standalone page that does NOT use `Base.astro`, noindexed; keep its public content in sync with `experience.astro`).
- **`src/layouts/Base.astro`** is the single shared layout: handles `<head>`/meta/OG tags (`public/og.png` is the share image), the sidebar/mobile nav shell, the ambient sound toggle, and a shared scroll-reveal `IntersectionObserver` (elements with class `rv` fade/slide in; respects `prefers-reduced-motion`). All pages wrap their content in `<Base>`.
- **Styling**: `src/styles/global.css` is only an `@import` entry point — the actual rules live in ordered partials in `src/styles/` (cascade order matters, don't reorder imports): `base.css` (tokens/reset/typography), `shell.css` (sidebar/mobile bar), `motion.css` (rv reveal/ripple/intro veil), `home.css` (hero/work grid/side work/kind words/footer), `case-study.css`, `experience.css`, `contact.css`, `misc.css` (404/password gate). No CSS modules/utility framework. Fonts are Google Fonts (Inter Tight, Instrument Serif, Noto Sans JP) loaded in `Base.astro`. Grep the partial, not global.css.
...
- **Interactive behaviors** live as modules in `src/scripts/`, each exporting an idempotent `init*()` bound on `astro:page-load` by the page that needs it: `ambient.js` (generative background sound engine driving the sidebar/mobile-bar toggle), `fluid-photo.js` (WebGL fluid distortion over images — used on `.work-thumb` on the top page and `.xp-photo` on experience; falls back to the plain `<img>` without WebGL), `kind-words.js` (typewriter reveal for the Kind Words quotes), `accordion.js` (WAAPI height animation for `details.xp-entry`), `private-mode.js` (password modal for the private resume detail), `load-more.js` (work-grid reveal), `haptics.js` (touch haptic feedback via the Vibration API — delegated listeners wired once in `Base.astro`, per-element overrides via `data-haptic`, result feedback via the exported `haptic()`; design vocabulary and rules in `docs/haptic-guidelines.md`). `[slug].astro` keeps its metric count-up inline; `HeroField.astro` contains its own WebGL dot-field script.
